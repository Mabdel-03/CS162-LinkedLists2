/* This code was written by Mahmoud Abdelmoneum 
 * on 4/12/2021 with the exception of Node.cpp and node.h 
 * Which were written by Aammya Sapra for the partner LinkedLists2 project
 */
...
//Outputs average GPA of all students to 2 decimal places
void Average(Node* next, float total, int count){
  float avg = 0.00;
  Node *traverse = next;
  if(traverse != NULL){
    count++; //increase count
    total += traverse->getStudent()->getGPA(); //add GPA's to total
    //recursively call function (backwards)
...
//checks for unique ID (example, cannot have 2 students with ID 1).
